...
const pengajuanbeliPrint = (ticket, ticket_detail) => {    
    var doc = new jsPDF()
    ...
    var tableData = [
        ['No', 'Jenis Produk', 'Jumlah', 'Harga', 'Deskripsi'],
    ];
    
    let i = 1
    ticket_detail.item_pengajuanbeli.forEach(item => {
        let row = [i, item.jenis_produk, item.jumlah, item.harga, item.deskripsi_produk]
        tableData.push(row)
        i += 1
    });

    // Define table column widths (optional)
    var columnWidths = [10, 50, 20, 40, 40]; // Adjust as needed
